...
def color_change2(modify_image, search_image, corner, rect, color):
...
def rand_color():
    b = random.randint(55, 225)
    g = random.randint(65, 225)
    r = random.randint(75, 225)

    return b, g, r



cv2.imread('/Users/ibnefarabishihab/Desktop/Course materials /COMS 575/HW2/hw2_files/p1_output.png')
cv2.imshow("in "+image)
...
for filename in os.listdir('Symbol_Cutouts'):
            current_letter = str(filename[0])
            letter_image = cv2.imread("Symbol_Cutouts/" + str(filename))
            ...
            for c in contours:
                ...
                color_change2(modify_im, ~img, (0, -1), (x1, y1, w, h), (0, 0, 255))

            cv2.imwrite("colored_"+image_file, modify_im)
        ...
        # cv2.imshow("v", vert)
            ...
            print("Row" + str(rows) + " Col" + str(cols))
        ...
        colors = [[(0, 0, 0) for i in range(cols)] for j in range(rows)]
        words_to_find = ["X", "O", "+", "/", "\\", "", ".", "-", "|",]
        ...
        cv2.imwrite("colo_" + image_file, image)
        cv2.imshow("Colo " + image_file, image)
        # cv2.waitKey(0)
