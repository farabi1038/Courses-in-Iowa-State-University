...
def study_C_fix_split(C_range): 
    ...
    maxScore=0
    for i in C_range:
        clf=sklearn.svm.SVC(kernel='linear',C=i,random_state=1).fit(X_train,y_train)
...
def study_C_gridCV(C_range):
    ...
    # your code here
    clf=GridSearchCV(model,paras).fit(X,y)
...
def study_C_and_sigma_gridCV(C_range, sigma_range):
    ...
    clf = GridSearchCV(model, paras).fit(X, y)
    best_C,best_sigma = clf.best_params_['C'],clf.best_params_['gamma']
